...
class SipSettingsWidget extends Component {
    static template = xml`
    <div class="sip-settings-container">
        <!-- Header Section -->
        <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
            <div>
                <h1 class="h3 fw-bold text-dark mb-1">Danh mục cấu hình SIP</h1>
        ...
        <div class="card search-container border-0 mb-4">
            <div class="card-body">
                ...
                <div class="table-responsive d-none d-lg-block">
                    <table class="table table-hover mb-0">
                        <thead class="bg-light">
                            <tr>
                                <th scope="col" class="px-4 py-3 text-uppercase small fw-semibold text-muted">Chương trình của SIP</th>
                                <th scope="col" class="px-4 py-3 text-uppercase small fw-semibold text-muted">Số kỳ bỏ lỡ tối đa</th>
                                <th scope="col" class="px-4 py-3 text-uppercase small fw-semibold text-muted">Số tiền tối thiểu/kỳ</th>
                                <th scope="col" class="px-4 py-3 text-uppercase small fw-semibold text-muted">Số kỳ duy trì tối thiểu</th>
                                <th scope="col" class="px-4 py-3 text-uppercase small fw-semibold text-muted">Mã chu kỳ</th>
                                <th scope="col" class="px-4 py-3 text-center text-uppercase small fw-semibold text-muted">Đầu tư nhiều lần</th>
                                <th scope="col" class="px-4 py-3 text-center text-uppercase small fw-semibold text-muted">Kích hoạt</th>
                                <th scope="col" class="px-4 py-3 text-center text-uppercase small fw-semibold text-muted">Thao tác</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <t t-if="!state.loading and state.settings.length === 0">
                                <tr>
                                    <td colspan="8" class="text-center py-5">
                                        <div class="mb-4"><i class="fas fa-cogs fa-4x text-muted opacity-50"></i></div>
                                        ...
                                        <button class="btn btn-primary" t-on-click="createNew"><i class="fas fa-plus me-2"></i>Tạo cấu hình SIP</button>
                                    </td>
                                </tr>
                            </t>
                            <t t-foreach="state.settings" t-as="setting" t-key="setting.id">
                                <tr class="align-middle">
                                    <td class="px-4 py-3 fw-semibold text-dark" t-esc="setting.sip_scheme_name"/>
                                    <td class="px-4 py-3 text-center" t-esc="setting.max_non_consecutive_periods"/>
                                    <td class="px-4 py-3"><span class="fw-semibold text-success" t-esc="formatCurrency(setting.min_monthly_amount)"/> <small>VND</small></td>
                                    <td class="px-4 py-3 text-center" t-esc="setting.min_maintenance_periods"/>
                                    <td class="px-4 py-3"><span class="badge bg-light text-dark fw-normal" t-esc="setting.cycle_code"/></td>
                                    ...
                                    <td class="px-4 py-3 text-center">
                                        <span t-if="setting.active" class="badge rounded-pill bg-success-subtle text-success px-3 py-2"><i class="fas fa-check-circle me-1"></i>Kích hoạt</span>
                                        <span t-else="" class="badge rounded-pill bg-danger-subtle text-danger px-3 py-2"><i class="fas fa-times-circle me-1"></i>Tạm dừng</span>
                                    </td>
                                    <td class="px-4 py-3 text-center">
                                        <div class="d-flex gap-2 justify-content-center">
                                            ...
                                            <button t-on-click="() => this.confirmDelete(setting.id, setting.sip_scheme_name)" class="btn btn-sm btn-outline-danger">
                        ...
                        <div class="card mb-2 border-0 shadow-sm">
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between align-items-start mb-2">
                                    <div>
                                        <h6 class="fw-bold mb-0" t-esc="setting.sip_scheme_name"/>
                                        <small class="text-muted" t-esc="setting.cycle_code"/>
                                    </div>
                                    <span t-if="setting.active" class="badge bg-success-subtle text-success">Kích hoạt</span>
                                    <span t-else="" class="badge bg-danger-subtle text-danger">Tạm dừng</span>
                                </div>
                                <div class="d-flex justify-content-between align-items-end">
                                    <div>
                                        <div class="small text-muted">Số tiền tối thiểu</div>
                                        <div class="fw-bold text-success" t-esc="formatCurrency(setting.min_monthly_amount)"/>
                                    </div>
                                    <div class="d-flex gap-2">
                                        <button t-on-click="() => this.handleEdit(setting.id)" class="btn btn-sm btn-light border px-3 py-2"><i class="fas fa-edit"></i></button>
                                        <button t-on-click="() => this.confirmDelete(setting.id, setting.sip_scheme_name)" class="btn btn-sm btn-outline-danger px-3 py-2"><i class="fas fa-trash"></i></button>
                                    </div>
                                </div>
                            </div>
                        </div>
