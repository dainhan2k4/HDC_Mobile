 ...
 */
console.log("WidgetMountingService loaded.");
    ...
    const service = {
        ...
        mountWhenReady(widgetClassName, containerId) {
            ...
            if (this.mountedWidgets.has(widgetKey)) {
                console.log(`WidgetMountingService: ${widgetClassName} already mounted in #${containerId}. Skipping.`);
                return;
            }
            ...
            if (existingInQueue) {
                console.log(`WidgetMountingService: ${widgetClassName} already queued for #${containerId}. Skipping.`);
                return;
            }
            ...
            if (!container) {
                ...
                console.log(`WidgetMountingService: Container #${containerId} not found. Skipping mount for ${widgetClassName}.`);
                return;
            }
            ...
            if (container.children.length > 0 && !container.querySelector('.spinner-border')) {
                console.log(`WidgetMountingService: Container #${containerId} already has content. Skipping mount for ${widgetClassName}.`);
                this.mountedWidgets.add(widgetKey);
                return;
            }

            console.log(`WidgetMountingService: Queued ${widgetClassName} for container #${containerId}.`);
        ...
        startMountingProcess() {
            console.log("WidgetMountingService: Starting mounting process...");
        ...
        tryToMountWidgets() {
            ...
            if (this.widgetsToMount.length === 0) {
                console.log("WidgetMountingService: All widgets mounted. Stopping process.");
            ...
            for (let i = this.widgetsToMount.length - 1; i >= 0; i--) {
                ...
                if (window.owl && WidgetClass && container) {
                    ...
                    if (this.mountedWidgets.has(widgetInfo.widgetKey)) {
                        console.log(`WidgetMountingService: ${widgetInfo.widgetClassName} already mounted. Removing from queue.`);
                        this.widgetsToMount.splice(i, 1);
                        continue;
                    }

                    console.log(`Mounting ${widgetInfo.widgetClassName}...`);
                    try {
                        ...
                        console.log(`${widgetInfo.widgetClassName} mounted successfully in #${widgetInfo.containerId}!`);
                        ...
                        console.error(`Error mounting ${widgetInfo.widgetClassName}:`, error);
                        // Remove from queue to prevent retrying a failed mount.
                        this.widgetsToMount.splice(i, 1);
                    }
                } else if (widgetInfo.attempts > this.MAX_ATTEMPTS) {
                    // Handle timeout.
                    console.error(`Failed to mount ${widgetInfo.widgetClassName} after ${this.MAX_ATTEMPTS} attempts. Conditions:`, {
                        owlReady: !!window.owl,
                        widgetClassReady: !!WidgetClass,
        ...
        clearMountedWidgets() {
            ...
            console.log("WidgetMountingService: Cleared mounted widgets tracking.");
        }
    };
