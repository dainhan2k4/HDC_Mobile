...
<div class="scheme-container">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
        <div>
            <h1 class="h3 fw-bold text-dark mb-1">Chương trình</h1>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb small mb-0">
                    <li class="breadcrumb-item"><a href="#" class="text-decoration-none text-muted"><i class="fas fa-home me-1"></i>Trở về</a></li>
                    <li class="breadcrumb-item active text-primary" aria-current="page">Danh mục chương trình</li>
                </ol>
            </nav>
        </div>
        ...
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    <thead>
                        <tr>
                            <th class="px-4 py-3 text-uppercase small fw-semibold text-muted">Tên chương trình</th>
                            <th class="px-4 py-3 text-uppercase small fw-semibold text-muted">Mã giao dịch</th>
                            <th class="px-4 py-3 text-uppercase small fw-semibold text-muted">Giá trị mua tối thiểu</th>
                            <th class="px-4 py-3 text-uppercase small fw-semibold text-muted">Quyền hạn</th>
                    ...
                    <tbody>
                        ...
                        <t t-if="!state.loading and state.schemes.length === 0">
                            <tr>
                                <td colspan="6" class="text-center py-5">
                                    <div class="mb-4"><i class="fas fa-tasks fa-4x text-muted opacity-50"></i></div>
                                    <h5 class="text-muted mb-3">Chưa có Chương trình nào</h5>
                                    <p class="text-muted mb-4">Bắt đầu bằng cách tạo chương trình đầu tiên của bạn.</p>
                                    <button class="btn btn-primary" t-on-click="createNew"><i class="fas fa-plus me-2"></i>Tạo chương trình</button>
                                </td>
                            </tr>
                        </t>
                        <t t-foreach="state.schemes" t-as="s" t-key="s.id">
                            <tr class="align-middle">
                                <td class="px-4 py-3">
                                    <div class="fw-semibold text-dark" t-esc="s.name"/>
                                </td>
                                <td class="px-4 py-3">
                                    <span class="badge bg-light text-dark fw-normal" t-esc="s.transaction_code"/>
                                </td>
                                <td class="px-4 py-3">
                                    <span class="fw-semibold text-success" t-esc="formatCurrency(s.min_purchase_value)"/>
                                ...
                                <td class="px-4 py-3">
                                    <!-- FIX START: Corrected the status check logic -->
                                    <span t-if="s.active_status === 'Kích hoạt'" class="badge rounded-pill bg-success-subtle text-success px-3 py-2">
                                        <i class="fas fa-check-circle me-1"></i>Kích hoạt
                                    </span>
                                    <span t-else="" class="badge rounded-pill bg-danger-subtle text-danger px-3 py-2">
                                        <i class="fas fa-times-circle me-1"></i>
                                        <t t-esc="s.active_status || 'Không kích hoạt'"/>
                                    </span>
                                    <!-- FIX END -->
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <div class="d-flex gap-2 justify-content-center">
                                        ...
                                        <button t-on-click="() => this.confirmDelete(s.id, s.name)" class="btn btn-sm btn-outline-danger">
