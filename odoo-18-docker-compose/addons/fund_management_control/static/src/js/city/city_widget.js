/** @odoo-module **/
// city_widget.js cho city
console.log("Loading CityWidget...");

const { Component, xml, useState, onMounted } = window.owl;

class CityWidget extends Component {
    static template = xml`
          ...
          <div class="card-body p-0">
            <table class="table table-hover mb-0">
                <thead class="table-light">
                    <tr>
                        <th class="px-3 py-2 text-uppercase small text-muted">Tên thành phố</th>
                        <th class="px-3 py-2 text-uppercase small text-muted">Mã thành phố</th>
                        <th class="px-3 py-2 text-uppercase small text-muted">Quốc gia</th>
                ...
                <tbody>
                    ...
                    <t t-if="!state.loading and state.cities.length === 0">
                        <tr>
                    ...
                    <t t-foreach="state.cities" t-as="city" t-key="city.id">
                        <tr class="align-middle">
                            <td class="px-3 py-2 fw-bold" t-esc="city.name"/>
                            <td class="px-3 py-2 text-muted" t-esc="city.code"/>
                            <td class="px-3 py-2" t-esc="city.country_name"/>
                            <td class="px-3 py-2">
                                <span t-if="city.active" class="badge rounded-pill bg-success-subtle text-success-emphasis p-2">
                                    Kích hoạt
                                </span>
                            ...
                            <td class="px-3 py-2 text-center">
                                <a href="#" t-on-click.prevent="() => this.handleEdit(city.id)" class="btn btn-sm btn-link text-primary text-decoration-none">
                                    <i class="fas fa-pencil-alt fa-lg"></i>
                                </a>
                                <a href="#" t-on-click.prevent="() => this.handleDelete(city.id)" class="btn btn-sm btn-link text-danger text-decoration-none">
    ...
    `;
    setup() {
        this.state = useState({ cities: [], searchTerm: "", loading: true, currentPage: 1, totalRecords: 0, limit: 10 });
        onMounted(() => this.loadData());
    }
    get totalPages() { return Math.ceil(this.state.totalRecords / this.state.limit); }
    async loadData() {
        this.state.loading = true;
        const url = `/get_city_data?page=${this.state.currentPage}&limit=${this.state.limit}&search=${encodeURIComponent(this.state.searchTerm.trim())}`;
        try {
            const res = await fetch(url);
            if (!res.ok) throw new Error("Tải dữ liệu thất bại");
            const data = await res.json();
            this.state.cities = data.records;
    ...
    handleEdit(id) { window.location.href = `/city/edit/${id}`; }
    createNew() { window.location.href = '/city/new'; }
}
