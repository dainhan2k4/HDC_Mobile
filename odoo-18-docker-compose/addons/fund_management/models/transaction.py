...
class Transaction(models.Model):
    ...
    def _compute_datetime_formatted(self):
        """Format datetime fields thành dd/MM/yyyy HH:mm:ss"""
        for record in self:
            # Format created_at
            if record.created_at:
                try:
                    if isinstance(record.created_at, datetime):
                        record.created_at_formatted = record.created_at.strftime('%d/%m/%Y %H:%M:%S')
                    elif isinstance(record.created_at, str):
                        # Parse string datetime
                        dt = fields.Datetime.from_string(record.created_at)
                        record.created_at_formatted = dt.strftime('%d/%m/%Y %H:%M:%S')
                    else:
            ...
            if record.date_end:
                try:
                    if isinstance(record.date_end, datetime):
                        record.date_end_formatted = record.date_end.strftime('%d/%m/%Y %H:%M:%S')
                    elif isinstance(record.date_end, str):
                        # Parse string datetime
                        dt = fields.Datetime.from_string(record.date_end)
                        record.date_end_formatted = dt.strftime('%d/%m/%Y %H:%M:%S')
                    else:
    ...
    def _update_investment(self):
        ...
        for tx in self:
            ...
            if tx.transaction_type == constants.TRANSACTION_TYPE_PURCHASE:
