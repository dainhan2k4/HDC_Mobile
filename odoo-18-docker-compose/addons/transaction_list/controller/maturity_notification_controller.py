...
class MaturityNotificationController(http.Controller):
    ...
    def confirm_sell(self, notification_id, token, **kwargs):
        ...
        try:
            ...
            if notification.investor_response != 'pending':
                return request.render('transaction_list.maturity_notification_response', {
        ...
        try:
            ...
            for notif in notifications:
                result.append({
                    ...
                    'maturity_date': notif.maturity_date.strftime('%Y-%m-%d') if notif.maturity_date else '',
                    'units': notif.units,
    ...
    @http.route('/api/transaction-list/send-maturity-notifications', type='json', auth='user')
    def send_maturity_notifications(self, **kwargs):
        ...
        try:
            ...
            result = request.env['transaction.maturity.notification'].check_maturity_dates()
    ...
    @http.route('/api/transaction-list/send-maturity-notifications-test', type='json', auth='user')
    def send_maturity_notifications_test(self, **kwargs):
        ...
        try:
            ...
            result = request.env['transaction.maturity.notification'].check_maturity_dates_for_test()
            ...
            _logger.error(f"Lỗi khi gửi thông báo đáo hạn (TEST): {str(e)}")
            return {
                'success': False,
                'message': f'Lỗi khi gửi thông báo đáo hạn: {str(e)}'
            }
    ...
    @http.route('/api/transaction-list/confirm-maturity-notification/<int:notification_id>', type='json', auth='user')
    def confirm_maturity_notification(self, notification_id, **kwargs):
        ...
        try:
            ...
            # Đơn giản chỉ cần browse lại với cùng ID, Odoo sẽ tự động load từ database
            notification = request.env['transaction.maturity.notification'].sudo().browse(notification_id)
            ...
            _logger.error(f"Lỗi khi xác nhận thông báo đáo hạn: {str(e)}")
            return {
                'success': False,
