...
class PartialMatchingController(http.Controller):
    """Controller cho Partial Matching Engine"""
    ...
    def create_engine(self, **kwargs):
        ...
        try:
            ...
            return request.make_response(
                json.dumps({
                    "success": True,
                    "message": f"Đã tạo engine cho quỹ {fund_id}",
                    "engine_id": engine.id,
            ...
            _logger.error(f"Error creating engine: {str(e)}")
            return request.make_response(
                json.dumps({
                    "success": False,
                    "message": str(e)
    ...
    def add_order(self, **kwargs):
        ...
        try:
            ...
            return request.make_response(
                json.dumps({
                    "success": True,
                    "message": f"Đã thêm lệnh và khớp {len(matched_pairs)} cặp",
            ...
            _logger.error(f"Error adding order: {str(e)}")
            return request.make_response(
                json.dumps({
                    "success": False,
                    "message": str(e)
    ...
    def process_all_orders(self, **kwargs):
        ...
        try:
            ...
            return request.make_response(
                json.dumps({
                    "success": True,
                    "message": f"Đã xử lý và khớp {len(matched_pairs)} cặp lệnh",
                    "matched_pairs": matched_pairs,
                    "engine_stats": {
                        "total_matches": engine.total_matches,
                        "total_partial_matches": engine.total_partial_matches,
            ...
            _logger.error(f"Error processing all orders: {str(e)}")
            return request.make_response(
                json.dumps({
                    "success": False,
                    "message": str(e)
    ...
    def clear_queue(self, **kwargs):
        ...
        try:
            ...
            _logger.error(f"Error clearing queue: {str(e)}")
            return request.make_response(
                json.dumps({
                    "success": False,
                    "message": str(e)
    ...
    def list_engines(self, **kwargs):
        ...
        try:
            ...
            _logger.error(f"Error listing engines: {str(e)}")
            return request.make_response(
                json.dumps({
                    "success": False,
                    "message": str(e)
    ...
    def cleanup_old_queues(self, **kwargs):
        ...
        try:
            ...
            return request.make_response(
                json.dumps({
                    "success": True,
                    "message": f"Đã dọn dẹp {cleaned_count} queue cũ (>{days} ngày)",
            ...
            _logger.error(f"Error cleaning up queues: {str(e)}")
            return request.make_response(
                json.dumps({
                    "success": False,
                    "message": str(e)
