...
export class NegotiatedOrdersComponent extends Component {
    ...
    <div class="order-book-container" t-attf-style="margin-top: 16px; position: relative; z-index: 0; overflow: visible;">
        <div class="ob-nav" style="display:flex; gap:12px; margin-bottom:16px; flex-wrap:wrap;">
            ...
            <a href="/completed-orders" style="text-decoration:none;"><span class="ob-pill">Khoản đầu tư đã khớp</span></a>
            <a href="#" style="text-decoration:none;"><span class="ob-pill ob-pill-active">Khoản đầu tư khớp theo thỏa thuận</span></a>
        </div>
        <style>
        ...
        <div class="order-book-header">
            <div class="header-center">
                <div class="fund-selector">
                    <label for="fund-select-neg">Chọn quỹ:</label>
                    <select id="fund-select-neg" t-on-change="onFundChange" class="form-control">
        ...
        <div class="order-book-content" t-attf-style="display:block; padding:20px;">
            <div class="order-box" style="background:#fff; border-radius:12px; box-shadow:0 4px 20px rgba(0,0,0,0.08);">
                <div class="order-section-header" style="background:linear-gradient(135deg,#22c55e,#16a34a)">
                    <h3><i class="fa fa-handshake-o"></i> Giao dịch khớp theo thỏa thuận</h3>
                ...
                <div class="order-list">
                    ...
                    <t t-if="!state.loading and state.orders.length > 0">
                        <table class="ob-table ob-table-negotiated" style="text-align: center; width: 100%;">
                            <thead>
                                <tr>
                                    <th style="text-align: center;">Quỹ</th>
                                    <th style="text-align: center;">Giá khớp</th>
                                    <th style="text-align: center;">Số lượng khớp</th>
                                    <th style="text-align: center;">Tổng giá trị</th>
                                    <th style="text-align: center;">Loại khớp</th>
                                    <th style="text-align: center;">Ngày khớp</th>
                                    <th style="text-align: center;">Trạng thái</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr t-foreach="getOrdersForPage()" t-as="o" t-key="o.id">
                                    <td style="text-align: center;"><t t-esc="o.fund_name || 'N/A'"/></td>
                                    <td style="text-align: center;"><t t-esc="formatPrice(o.matched_price)"/></td>
                                    <td style="text-align: center;"><t t-esc="formatUnits(o.matched_quantity)"/></td>
                                    <td style="text-align: center;"><t t-esc="formatAmount(o.total_value)"/></td>
                                    <td style="text-align: center;">
                                        <t t-if="o.buy_user_type === 'market_maker' and o.sell_user_type === 'market_maker'">NTL - NTL</t>
                                        <t t-elif="o.buy_user_type === 'market_maker' or o.sell_user_type === 'market_maker'">NTL - Nhà đầu tư</t>
                                        <t t-else="">Nhà đầu tư - Nhà đầu tư</t>
                                    </td>
                                    <td style="text-align: center;"><t t-esc="formatDateTime(o.match_date)"/></td>
    ...
    async refreshData() {
        ...
        try {
            const res = await fetch('/api/transaction-list/negotiated', { 
                method: 'POST', 
                headers: { 'Content-Type': 'application/json' }, 
            ...
            if (data.success && data.data) {
                ...
                console.error('Error loading negotiated orders:', data.message || 'Unknown error');
                this.state.orders = [];
                this.updatePagination();
            }
            ...
            console.error('Error refreshing negotiated orders:', error);
            this.state.orders = [];
        } finally {
    ...
    async rotateToNextFund() {
        ...
        const selectElement = document.getElementById('fund-select-neg');
        if (selectElement) {
...
window.NegotiatedOrdersComponent = NegotiatedOrdersComponent;
