...
def get_service_from_env(env) -> PayOSService:
    ...
    client_id = params.get_param('payos.client_id', os.getenv('PAYOS_CLIENT_ID', ''))
    api_key = params.get_param('payos.api_key', os.getenv('PAYOS_API_KEY', ''))
    checksum_key = params.get_param('payos.checksum_key', os.getenv('PAYOS_CHECKSUM_KEY', ''))
    base_url = os.getenv('PAYOS_BASE_URL')
    return PayOSService(client_id, api_key, checksum_key, base_url)
