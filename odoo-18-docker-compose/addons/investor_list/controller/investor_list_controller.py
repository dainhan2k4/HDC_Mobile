...
class InvestorListController(http.Controller):
    ...
    def investor_list_page(self, **kwargs):
        ...
        for record in investor_records:
            investor_data.append({
                ...
                'trang_thai_tk_dau_tu': record.trang_thai_tk_dau_tu or '',
                'ho_so_goc': record.ho_so_goc or '',
    ...
    def update_investor(self, investor_id, **kwargs):
        try:
            ...
            if 'trang_thai_tk_dau_tu' in data:
                update_data['trang_thai_tk_dau_tu'] = data['trang_thai_tk_dau_tu']
            if 'ho_so_goc' in data:
                update_data['ho_so_goc'] = data['ho_so_goc']
            if 'status' in data:
            ...
            response_data = {
                ...
                'trang_thai_tk_dau_tu': investor.trang_thai_tk_dau_tu or '',
                'ho_so_goc': investor.ho_so_goc or '',
    ...
    @http.route('/api/investor_list/<int:investor_id>', type='http', auth='user', methods=['DELETE'], csrf=False)
    def delete_investor(self, investor_id, **kwargs):
        try:
            investor = request.env['investor.list'].browse(investor_id)
            if not investor.exists():
                return json.dumps({'error': 'Nhà đầu tư không tồn tại'})
            ...
            return json.dumps({'success': True, 'message': 'Xóa nhà đầu tư thành công'})
        except Exception as e:
            return json.dumps({'error': str(e)})
