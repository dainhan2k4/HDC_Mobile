...
class PersonalProfileController(http.Controller):
    ...
    def get_currencies(self, **kwargs):
        try:
            ...
            data = [{'id': c.id, 'name': c.name, 'symbol': c.symbol} for c in currencies]
            return Response(json.dumps(data), content_type='application/json')
        except Exception as e:
            return Response(json.dumps({'error': str(e)}), content_type='application/json', status=500)
        ...
        try:
            ...
            if personal_profiles:
                for profile in personal_profiles:
                    id_front_url = f"/web/image?model=investor.profile&field=id_front&id={profile.id}" if profile.id_front else ''
                    id_back_url = f"/web/image?model=investor.profile&field=id_back&id={profile.id}" if profile.id_back else ''
                    data.append({
                        'id': profile.id,
            ...
            if verification_profiles:
                for profile in verification_profiles:
                    data.append({
                        ...
                        'contract_email': profile.partner_id.email,
        ...
        try:
            ...
            if address_data:
                address_vals = {
                    ...
                    'state_id': int(address_data.get('state')) if address_data.get('state') else False,
                    'zip': address_data.get('zip'),
                    'country_id': int(address_data.get('country_id')) if address_data.get('country_id') else False,
                }
