...
export class ReportContractSummaryWidget extends Component {
    static template = xml`
            <div class="report-contract-statistics-container">
                ...
                <h1 class="report-contract-statistics-title">Báo cáo Tổng hợp Hợp đồng</h1>
                <p class="report-contract-statistics-subtitle">Tổng hợp hợp đồng mua bán</p>
                </div>

            <!-- Navigation (JS-rendered like transaction_list) -->
                    ...
                    <table class="report-contract-statistics-table">
                        <thead>
                            <tr>
                                ...
                                <th style="font-size: 0.8rem; font-weight: 600; width: 200px;">Số Hợp đồng</th>
                                ...
                                <th style="font-size: 0.8rem; font-weight: 600; width: 120px;">Ngày mua</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 120px;">Ngày thanh toán</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 100px;">Số lượng</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 120px;">Giá mua</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 120px;">Thành tiền</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 80px;">Kỳ hạn</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 80px;">Lãi Suất</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 80px;">Số Ngày</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 150px;">Tiền lãi dự kiến khi đến hạn</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 150px;">Giá bán lại dự kiến theo HĐ</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 150px;">Gốc + lãi dự kiến khi đến hạn</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 150px;">Ngày bán lại dự kiến theo HĐ</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 120px;">Ngày đến hạn</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 120px;">Ngày bán lại</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 150px;">Ngày thanh toán bán lại</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 100px;">LS bán lại</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 120px;">Tiền lãi</th>
                                <th style="font-size: 0.8rem; font-weight: 600; width: 120px;">Gốc + lãi</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr t-if="!state.records || state.records.length === 0">
                                <td colspan="23" class="text-center py-3" style="font-size: 0.85rem;">
                                    <i class="fas fa-inbox me-2"></i>Không có dữ liệu tổng hợp hợp đồng
                                </td>
                            </tr>
                            <tr t-foreach="state.records" t-as="record" t-key="record.id">
                                ...
                                <td style="font-size: 0.8rem;" t-esc="record.ngay_mua || ''"/>
                                <td style="font-size: 0.8rem;" t-esc="record.ngay_thanh_toan || ''"/>
                                <td style="font-size: 0.8rem;" t-esc="record.so_luong || ''"/>
                                <td style="font-size: 0.8rem;" class="text-right price-cell" t-esc="this.formatNumber(record.gia_mua)"/>
                                <td style="font-size: 0.8rem;" class="text-right price-cell" t-esc="this.formatNumber(record.thanh_tien)"/>
                                <td style="font-size: 0.8rem;">
                                    <span class="chip chip-term" t-esc="record.ky_han || 0"/>
                                </td>
                                <td style="font-size: 0.8rem;">
                                    <span class="chip chip-rate" t-esc="record.lai_suat || 0"/>
                                </td>
                                <td style="font-size: 0.8rem;">
                                    <span class="chip chip-days" t-esc="record.so_ngay || 0"/>
                                </td>
                                <td style="font-size: 0.8rem;" class="text-right price-cell" t-esc="this.formatNumber(this.mround(record.tien_lai_du_kien, 50))"/>
                                <td style="font-size: 0.8rem;" class="text-right price-cell" t-esc="this.formatNumber(this.mround(record.gia_ban_lai_du_kien, 50))"/>
                                <td style="font-size: 0.8rem;" class="text-right price-cell" t-esc="this.formatNumber(this.mround(record.goc_lai_du_kien, 50))"/>
                                <td style="font-size: 0.8rem;" t-esc="record.ngay_ban_lai_du_kien || ''"/>
                                <td style="font-size: 0.8rem;" t-esc="record.ngay_den_han || ''"/>
                                <td style="font-size: 0.8rem;" t-esc="record.ngay_ban_lai || ''"/>
                                <td style="font-size: 0.8rem;" t-esc="record.ngay_thanh_toan_ban_lai || ''"/>
                                <td style="font-size: 0.8rem;" t-esc="record.ls_ban_lai || ''"/>
                                <td style="font-size: 0.8rem;" t-esc="record.tien_lai || ''"/>
                                <td style="font-size: 0.8rem;" t-esc="record.goc_lai || ''"/>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    setup() {
        ...
        onMounted(() => {
            console.log('ReportContractSummaryWidget OWL component mounted');
            this.initDropdown();
    ...
    async loadData() {
        ...
        try {
            ...
            // Đồng bộ trang và kích thước trang nếu backend trả về
    ...
    async exportPdf() {
        ...
        try {
            ...
            const response = await fetch(`/report-contract-summary/export-pdf?${params.toString()}`, {
                method: 'GET',
                headers: {
            ...
            a.href = url;
            a.download = `report_contract_summary_${new Date().toISOString().split('T')[0]}.pdf`;
    ...
    async exportXlsx() {
        ...
        try {
            ...
            const response = await fetch(`/report-contract-summary/export-xlsx?${params.toString()}`, {
                method: 'GET',
                headers: {
            ...
            const a = document.createElement('a');
            a.href = url;
            a.download = `report_contract_summary_${new Date().toISOString().split('T')[0]}.xlsx`;
            document.body.appendChild(a);
            a.click();
            window.URL.revokeObjectURL(url);
            document.body.removeChild(a);
