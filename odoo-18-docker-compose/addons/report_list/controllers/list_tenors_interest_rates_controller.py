...
class ListTenorsInterestRatesController(http.Controller):
    ...
    def get_report_data(self, filters, page, limit, search_values=None, **kws):
        ...
        try:
            ...
            records = request.env['nav.term.rate'].sudo().search(domain, limit=int(limit), offset=offset)
            total_records = request.env['nav.term.rate'].sudo().search_count(domain)
    ...
    def export_pdf(self, **kw):
        ...
        try:
            ...
            records = request.env['nav.term.rate'].sudo().search(domain)
