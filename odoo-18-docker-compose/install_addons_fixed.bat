@echo off
REM Script cài đặt tất cả custom addons cho Odoo 18 trên Windows (Fixed version)

echo 🚀 Bắt đầu cài đặt custom addons...

REM Danh sách các addons cần install (loại bỏ stock_market_data vì không tồn tại)
...
echo.
echo ⚠️  Lưu ý: Nếu database p2p chưa tồn tại, vui lòng tạo database trước qua Odoo web interface
echo    Truy cập: http://localhost:11018
echo.

REM Cài đặt addons vào database p2p với đầy đủ thông tin kết nối
echo 🔄 Đang cài đặt addons...
docker exec odoo-18-docker-compose-odoo18-1 odoo -c /etc/odoo/odoo.conf -d p2p --db_host=db --db_port=5432 --db_user=odoo --db_password=odoo18@2024 -i %ADDONS% --stop-after-init

if %ERRORLEVEL% EQU 0 (
    echo ✅ Hoàn thành cài đặt addons!
) else (
    echo ⚠️  Có lỗi xảy ra. Có thể database p2p chưa tồn tại.
    echo    Vui lòng:
    echo    1. Truy cập http://localhost:11018
    echo    2. Tạo database p2p (nếu chưa có)
