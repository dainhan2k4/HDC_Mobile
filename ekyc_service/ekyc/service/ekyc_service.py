...
class EKYCService:
    ...
    def __init__(self):
        ...
        self.CORNER_MODEL = ultralytics.YOLO(str(WEIGHTS_DIR / "corner.pt"))
        self.CORNER_BACK = yolov5.load(str(WEIGHTS_DIR / "corner_back.pt"))
        self.CONTENT_MODEL = yolov5.load(str(WEIGHTS_DIR / "content.pt"))
        self.CONTENT_BACK = ultralytics.YOLO(str(WEIGHTS_DIR / "content_back.pt"))
        self.NEW_CONTENT_BACK = ultralytics.YOLO(str(WEIGHTS_DIR / "new_content_back.pt"))
    ...
    def process_detection(self, image_path, destination):
        ...
        lm = landmarks[0]
        try:
            detected_orientation = self.orientation_detector.detect(lm)
            print(f"Raw detected orientation: {detected_orientation}")
            print(f"Expected by client: {destination}")
            
            # Map orientation to match client expectations
            # Test direct mapping first
            orientation_map = {
                'front': 'center',
                'left': 'left',    # Direct mapping 
                'right': 'right',  # Direct mapping
            }
            
            mapped_orientation = orientation_map.get(detected_orientation, detected_orientation)
            print(f"Mapped orientation: {mapped_orientation}")
            print(f"Will match? {mapped_orientation} == {destination}")
            
            # Handle 'up' direction - currently not supported by face detector
            if destination == 'up':
                # For 'up' direction, accept any frontal-ish orientation
                match = detected_orientation == 'front'
                print(f"Up direction requested, frontal check: {match}")
            else:
                match = (mapped_orientation == destination)
                print(f"Match check: {mapped_orientation} == {destination} -> {match}")
            
        except Exception as e:
            print(f"Detection error: {e}")
            detected_orientation = None
            mapped_orientation = None
            match = False
